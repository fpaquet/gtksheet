...
/* buildSheetRowEmpty() -- build all the items in a row of ProgSheet,
   but leave them at their default values.  */
void buildSheetRowEmpty(unsigned row)
{
  ...
  /* Get the main option menu */
  optionMenu = GTK_OPTION_MENU(gtk_option_menu_new());

  /* Build the step type dropdown menu itself */
  menu = GTK_MENU(gtk_menu_new());
  gtk_menu_set_tearoff_state(menu, FALSE);

  gtk_widget_show(menuItem = gtk_menu_item_new_with_label("Ramp Up"));
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuItem);

  gtk_widget_show(menuItem = gtk_menu_item_new_with_label("Ramp Down"));
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuItem);

  gtk_widget_show(menuItem = gtk_menu_item_new_with_label("Soak"));
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuItem);

  gtk_widget_show(menuItem = gtk_menu_item_new_with_label("Action"));
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuItem);

  gtk_widget_show(menuItem = gtk_menu_item_new_with_label("Wait Until"));
  gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuItem);

  /* Set the menu to be what pops up within the option menu, and
     start us out at the first item, the empty string. */
  gtk_option_menu_set_menu(optionMenu, GTK_WIDGET(menu));
  gtk_option_menu_set_history(optionMenu, 0);

  /* Put it in the first column of the requested row, and make
     sure we can see everything. */
  gtk_widget_show(GTK_WIDGET(optionMenu));
  gtk_sheet_attach(GTK_SHEET(sheets[0]),
		   GTK_WIDGET(optionMenu), row, 0, GTK_FILL,
		   GTK_FILL, 5, 5);

  /* Add the insert and delete buttons, with the signal handlers. */
  rowButton = gtk_button_new_with_label("Insert New");
  gtk_widget_show(rowButton);
  gtk_sheet_attach(GTK_SHEET(sheets[0]), rowButton, row, 5, GTK_FILL,
		   GTK_FILL, 5, 5);
  
  rowButton = gtk_button_new_with_label("Delete");
  gtk_widget_show(rowButton);
  gtk_sheet_attach(GTK_SHEET(sheets[0]), rowButton, row, 6, GTK_FILL,
		   GTK_FILL, 5, 5);
...
/* sigAddClicked -- callback handler for Add button clicks. */
void sigAddClicked(GtkButton *button, gpointer *userData)
{
  /* Insert a row just before the last row, and build it out. */
  gtk_sheet_insert_rows(sheets[0],
			gtk_sheet_get_rows_count(sheets[0])-1, 1);
 
  buildSheetRowEmpty(13); //gtk_sheet_get_rows_count(sheets[0])-2);

  /* Put some text in column 4 of this row for debugging */
  gtk_sheet_set_cell_text(sheets[0], gtk_sheet_get_rows_count(sheets[0])-2,
			  4, "added row");
...
*/
 ...
 show_button=gtk_button_new_with_label("Add row below");
...
int main(int argc, char *argv[]) 
{
  ...
  for(i=0; i<NUM_SHEETS; i++){
   ...
   sheets[i]=gtk_sheet_new(15,26,title[i]);
