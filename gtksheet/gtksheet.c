...
#   define GTK_SHEET_DEBUG_EXPOSE   0    /* 0,1,2 */
...
enum _GtkSheetFlags
{
    ...
    GTK_SHEET_SELECTION_START_POSSIBLE  = 1 << 11, /* button pressed, not yet moved */
    GTK_SHEET_MOUSE_BUTTON1_DOWN  = 1 << 12, /* button down, not yet released */
};
...
#define SELECTION_START_POSSIBLE(sheet) \
    (GTK_SHEET_FLAGS (sheet) & GTK_SHEET_SELECTION_START_POSSIBLE)

#define SELECTION_START_POSSIBLE(sheet) \
    (GTK_SHEET_FLAGS (sheet) & GTK_SHEET_SELECTION_START_POSSIBLE)

#define MOUSE_BUTTON1_DOWN(sheet) \
    (GTK_SHEET_FLAGS (sheet) & GTK_SHEET_MOUSE_BUTTON1_DOWN)
...
#if 0
/**
 * _g_list_check - check GList consistency
 * 
 * @param g_list the list to check
 */
static void _g_list_check(GList *g_list)
{
    GList *p = g_list;
...
#define DRAG_WIDTH 6
#define CLICK_CELL_MOTION_TOLERANCE 6  /* pixel */
...
/**
 * _gtk_sheet_first_focus_colidx:
 * 
 * find index of leftmost focusable cell (visible, can_focus)
 * 
 * same as _HUNT_FOCUS_RIGHT() macro
 * 
...
static gboolean gtk_sheet_touch_event_handler(GtkWidget *widget,
    GdkEventTouch *event);
...
static gboolean gtk_sheet_focus_in_event(GtkSheet *sheet,
    GdkEventFocus *event)
{
    ...
    g_debug("%s(%d): col %d inSel %s inResize %s inDrag %s selStPo %s mseB1 %s",
	__FUNCTION__, __LINE__,
	sheet->drag_cell.col, 
	GTK_SHEET_IN_SELECTION(sheet) ? "Yes" : "No",
	GTK_SHEET_IN_RESIZE(sheet) ? "Yes" : "No",
	GTK_SHEET_IN_DRAG(sheet) ? "Yes" : "No",
        SELECTION_START_POSSIBLE(sheet) ? "Yes" : "No",
        MOUSE_BUTTON1_DOWN(sheet) ? "Yes" : "No"
	);
#endif

    /* when klicking on the sheet
       BUTTON1_EVENT will be issued before FOCUS_IN_EVENT
       so click will be handled via gtk_sheet_button_press_handler()
       we only need to handle keypress events here
       */
    ...
    if (row < 0 || col < 0)
    {
        row = _gtk_sheet_first_visible_rowidx(sheet, 0);
        col = _gtk_sheet_first_focus_colidx(sheet, 0);
    }
...
gtk_sheet_class_init(GtkSheetClass *klass)
{
    ...
    widget_class->focus_in_event = gtk_sheet_focus_in_event;
    ...
    widget_class->touch_event = gtk_sheet_touch_event_handler;
...
#if GTK_SHEET_DEBUG_SIGNALS > 0
    widget_class->focus_out_event = gtk_sheet_debug_focus_out_event;
...
#if GTK_SHEET_DEBUG_MOUSE > 0
	g_debug("%s(%d): inSel %s inResize %s inDrag %s selStPo %s", 
	    __FUNCTION__, __LINE__, 
            GTK_SHEET_IN_SELECTION(sheet) ? "Yes" : "No",
            GTK_SHEET_IN_RESIZE(sheet) ? "Yes" : "No",
            GTK_SHEET_IN_DRAG(sheet) ? "Yes" : "No",
...
*/
    ...
    GTK_SHEET_SET_FLAGS(sheet, GTK_SHEET_MOUSE_BUTTON1_DOWN);
...
gtk_sheet_click_cell(GtkSheet *sheet, gint row, gint col, gboolean *veto)
{
    *veto = TRUE;
    ...
    g_debug("%s(%d): %p called, r %d c %d inSel %s inResize %s inDrag %s selStPo %s",
        __FUNCTION__, __LINE__, sheet, row, col, 
        GTK_SHEET_IN_SELECTION(sheet) ? "Yes" : "No",
        GTK_SHEET_IN_RESIZE(sheet) ? "Yes" : "No",
        GTK_SHEET_IN_DRAG(sheet) ? "Yes" : "No",
        SELECTION_START_POSSIBLE(sheet) ? "Yes" : "No"
	...
	*veto = FALSE;
    ...
    {
        if (GTK_SHEET_IN_SELECTION(sheet)) // set by button_press_handler
        {
	...
	else
	{
	    ...
	    g_debug("%s(%d): row %d col %d ar %d ac %d", 
            ...
            {
#if GTK_SHEET_DEBUG_CLICK > 0
                g_debug("%s(%d): row %d col %d calling deactivate, inSel %s inResize %s inDrag %s selStPo %s", 
                    __FUNCTION__, __LINE__, 
		    row, col,
		    GTK_SHEET_IN_SELECTION(sheet) ? "Yes" : "No",
		    GTK_SHEET_IN_RESIZE(sheet) ? "Yes" : "No",
...
#if GTK_SHEET_DEBUG_CLICK > 0
	    g_debug("%s(%d): row %d col %d back from deactivate, inSel %s inResize %s inDrag %s selStPo %s", 
		__FUNCTION__, __LINE__,
		row, col,
		GTK_SHEET_IN_SELECTION(sheet) ? "Yes" : "No",
		GTK_SHEET_IN_RESIZE(sheet) ? "Yes" : "No",
    ...
    GtkWidget *widget, GdkEventButton *event)
{
...
#if GTK_SHEET_DEBUG_MOUSE > 0
	g_debug("%s(%d): click cell r/c %d %d ar/ac %d %d inSel %s inResize %s inDrag %s selStPo %s mseB1 %s", 
	    __FUNCTION__, __LINE__, 
	    row, column,
	    sheet->active_cell.row, sheet->active_cell.col,
            GTK_SHEET_IN_SELECTION(sheet) ? "Yes" : "No",
    ...
    if (SELECTION_START_POSSIBLE(sheet))
    {
#if GTK_SHEET_DEBUG_MOUSE > 0
        g_debug("%s(%d): UNSET_FLAGS(SELECTION_START_POSSIBLE)",
            __FUNCTION__, __LINE__);
#endif
        GTK_SHEET_UNSET_FLAGS(sheet, GTK_SHEET_SELECTION_START_POSSIBLE);
    }
...
gtk_sheet_motion_handler(GtkWidget *widget, GdkEventMotion *event)
{
    ...
    g_debug("%s(%d): (%d,%d) inSel %s inResize %s inDrag %s selStPo %s state mod %u but %u", 
	__FUNCTION__, __LINE__,
	x, y,
	GTK_SHEET_IN_SELECTION(sheet) ? "Yes" : "No",
	GTK_SHEET_IN_RESIZE(sheet) ? "Yes" : "No",
    ...
    {
#if GTK_SHEET_DEBUG_MOUSE > 0
	g_debug("%s(%d): check mouse button1 inSel %s inResize %s inDrag %s selStPo %s mseB1 %s", 
	    __FUNCTION__, __LINE__,
	    GTK_SHEET_IN_SELECTION(sheet) ? "Yes" : "No",
	    GTK_SHEET_IN_RESIZE(sheet) ? "Yes" : "No",
	    GTK_SHEET_IN_DRAG(sheet) ? "Yes" : "No",
...
#if GTK_SHEET_DEBUG_MOUSE > 0
	g_debug("%s(%d): check dx/dy %d/%d inSel %s inResize %s inDrag %s selStPo %s mseB1 %s",
	    __FUNCTION__, __LINE__,
	    dx, dy,
	    GTK_SHEET_IN_SELECTION(sheet) ? "Yes" : "No",
	    GTK_SHEET_IN_RESIZE(sheet) ? "Yes" : "No",
	...
	if (!GTK_SHEET_IN_SELECTION(sheet)  /* not selecting */
		...
		|| abs(dy) > CLICK_CELL_MOTION_TOLERANCE)
            && SELECTION_START_POSSIBLE(sheet)
            )
	{
#if GTK_SHEET_DEBUG_MOUSE > 0
	    g_debug("%s(%d): SET_FLAGS(IN_SELECTION)",
                __FUNCTION__, __LINE__);
#endif
            if (sheet->state != GTK_SHEET_NORMAL)
            {
...
/*
 * gtk_sheet_touch_event_handler:
 * 
 * this is the #GtkSheet widget class "touch-event" handler
 * 
 * @param widget the #GtkSheet
 * @param event  the GdkEventTouch which triggered this signal
 * 
 * @return TRUE to stop other handlers from being invoked for the event. FALSE to propagate the event further.
 * @param widget
 * @param event
 ...
 */
static gboolean
gtk_sheet_touch_event_handler(GtkWidget *widget, GdkEventTouch *event)
{
    GtkSheet *sheet;
    GdkModifierType mods;
    gint x, y, row, column;
...
#if GTK_SHEET_DEBUG_TOUCH > 0
    g_debug("%s(%d): %s", 
        __FUNCTION__, __LINE__,
        event->type == GDK_TOUCH_BEGIN ? "TOUCH_BEGIN" :
        event->type == GDK_TOUCH_UPDATE ? "TOUCH_UPDATE" :
        event->type == GDK_TOUCH_END ? "TOUCH_END" :
        event->type == GDK_TOUCH_CANCEL ? "TOUCH_CANCEL" :
        "unknown event type");
#endif
...
#if GTK_SHEET_DEBUG_TOUCH > 0
            g_debug("%s(%d): on sheet ev(%f, %f) evr(%f, %f) dev(%d, %d)", 
                __FUNCTION__, __LINE__,
                event->x, event->y, event->x_root, event->y_root, x, y);
#endif
    ...
    {
#if 0
        /* The code below works basically.
           - resizing selection with touch screen interferes with touch scrolling
           - low touch point precision leads to large flickering areas
           This is not a wanted feature.
           */
        ...
        x = event->x;
        y = event->y;
        mods = GDK_BUTTON1_MASK;  // should be set by driver

#if GTK_SHEET_DEBUG_TOUCH > 1
        g_debug("%s(%d): check mouse button1", __FUNCTION__, __LINE__);
#endif
    ...
    return(FALSE);  /* chain up to global touch handler*/
}
...
{
#if GTK_SHEET_DEBUG_SELECTION > 0
    g_debug("%s(%d): r/c %d/%d PIN %d/%d inSel %s inResize %s inDrag %s selStPo %s", 
	__FUNCTION__, __LINE__,
	row, column,
        sheet->selection_pin.row, sheet->selection_pin.col,
	GTK_SHEET_IN_SELECTION(sheet) ? "Yes" : "No",
...
gtk_sheet_put(GtkSheet *sheet, GtkWidget *child, gint x, gint y)
{
    ...
    sheet->children = g_list_prepend(sheet->children, child_info);
    g_object_ref(child);
    ...
    gint row, gint col)
{
    ...
    GtkSheetChild *child = gtk_sheet_put(sheet, widget, area.x, area.y);
    ...
    gint ypadding)
{
    ...
    sheet->children = g_list_prepend(sheet->children, child);
    ...
    gint row, gint col)
{
	...
	GtkWidget *child_widget = gtk_bin_get_child(
            GTK_BIN(colobj->col_button));

        if (child_widget)
	{
	    DEBUG_WIDGET_CONTAINER_REMOVE(
		colobj->col_button, child_widget);

            /* gtk_container_remove invokes gtk_sheet_remove_handler()
               which invalidates child_widget */
	    gtk_container_remove(
		GTK_CONTAINER(colobj->col_button), child_widget);
	}
    ...
    /* attach to row title button */

    GtkSheetChild *child = g_new(GtkSheetChild, 1);
    child->widget = widget;
    child->x = 0;
    child->y = 0;
    child->attached_to_cell = TRUE;
...
gtk_sheet_position_child(GtkSheet *sheet, GtkSheetChild *child)
{
...
#if GTK_SHEET_DEBUG_CHILDREN > 0
    g_debug("%s(%d): child %s %p visible %d realized %d mapped %d", 
        __FUNCTION__, __LINE__,
        G_OBJECT_TYPE_NAME(child->widget), child->widget, 
        gtk_widget_get_visible(GTK_WIDGET(child->widget)),
        gtk_widget_get_realized(GTK_WIDGET(child->widget)),
        gtk_widget_get_mapped(GTK_WIDGET(child->widget))
        );
#endif
    ...
    if (child->attached_to_cell)
    {
	if (child->row < MIN_VIEW_ROW(sheet)  /* outside visible area */
    ...
    //gtk_widget_queue_draw(child->widget);  /* disabled 05.03.22/fp 309725 */
}
...
_gtk_sheet_child_hide(GtkSheetChild *child)
{
    ...
    if (!gtk_widget_get_visible(child->widget)) return;
