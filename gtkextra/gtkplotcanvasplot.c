				 	 ...
				 	 GtkPlotCanvasChild *child)
{
  ...
  switch(cplot->pos){
    ...
    case GTK_PLOT_CANVAS_PLOT_IN_GRADIENT:
      gtk_plot_data_move_gradient(cplot->data, (new_x - plot->x)*canvas->pixmap_width/plot->internal_allocation.width, (new_y - plot->y)*canvas->pixmap_height/plot->internal_allocation.height);
      child->drag_area = canvas->drag_area;
      break;
