...
static void fixup_range(GtkSheet *sheet, GtkSheetRange *range)
{
    ...
    POSSIBLE_XDRAG(GtkSheet *sheet, gint x, gint *drag_column)
{
    ...
    {
        while (column > 0 && !sheet->column[column-1]->is_visible) column--;
    ...
    POSSIBLE_YDRAG(GtkSheet *sheet, gint y, gint *drag_row)
{
    ...
    {
        while (row > 0 && !sheet->row[row-1].is_visible) row--;
                              ...
                              const gchar   *type)
{
    ...
    g_debug("gtk_sheet_buildable_add_child: %s type %s",
            name ? name : "NULL", 
            type ? type : "NULL" );
#endif
...
{
#ifdef GTK_SHEET_DEBUG
    g_debug("gtk_sheet_buildable_construct_child: %s", name);
#endif
    return(NULL);
}
    ...
    gtk_sheet_column_get_type (void)
{
    ...
    if (!sheet_column_type)
    {
        ...
        sheet_column_type = g_type_register_static (GTK_TYPE_WIDGET,
                                             "GtkSheetColumn",
                                             &sheet_column_info,
                                             0);
    }
                            ...
                            GParamSpec   *pspec)
{
    ...
    switch (property_id)
    {
        ...
        case PROP_GTK_SHEET_NROWS:
            {
                uint newval = g_value_get_int(value);
                            ...
                            GParamSpec *pspec)
{
    ...
    switch (property_id)
    {
        ...
        case PROP_GTK_SHEET_NROWS:
            g_value_set_int (value, sheet->maxrow);
            break;
                                   ...
                                   GParamSpec   *pspec)
{
    ...
    switch(property_id)
    {
        case PROP_GTK_SHEET_COLUMN_POSITION:
            /* note: glade/gtkbuilder will try to set this property before the column
               gets added to the sheet */
            {
                GtkSheetColumn *swapcol;
                uint newval = g_value_get_int(value);

                if (!sheet) return;
                if (newval < 0 || newval > sheet->maxcol) return;
...
#ifdef GTK_SHEET_DEBUG
                g_debug("gtk_sheet_column_set_property: swapping column %d/%d", newval, pos);
#endif

                /* method: swap */
                swapcol = sheet->column[newval];
                sheet->column[newval] = sheet->column[pos];
                sheet->column[pos] = swapcol;
        ...
        case PROP_GTK_SHEET_COLUMN_LABEL:
            {
                ...
                gtk_sheet_column_button_add_label(sheet, pos, g_value_get_string(value));
            }
            break;
        ...
        case PROP_GTK_SHEET_COLUMN_LABEL:
            {
                ...
                g_value_set_string (value, gtk_sheet_column_button_get_label(sheet, pos));
            }
            break;
...
*/
    ...
    if (drawing_range.coli == sheet->maxcol)  /* clear outer area beyond rightmost column */
    {
        if (sheet->maxcol >= 0)
        {
    ...
    if (drawing_range.rowi == sheet->maxrow)  /* clear outer area beyond last row */
    {
        area.x=0;
        if (sheet->maxrow >= 0)
        {
    ...
    gtk_sheet_deactivate_cell(GtkSheet *sheet)
{
    ...
    if (sheet->active_cell.row < 0 || sheet->active_cell.row > sheet->maxrow) return(TRUE);
    if (sheet->active_cell.col < 0 || sheet->active_cell.col > sheet->maxcol) return(TRUE);
...
*/
    ...
    switch (key->keyval)
    {
        ...
        case GDK_KP_Enter:
            ...
            {
                row = row + scroll;
                while (row < sheet->maxrow && !sheet->row[row].is_visible) row++;
        ...
        case GDK_ISO_Left_Tab:
            ...
            {
                col = col - scroll;
                while (col>0 && !sheet->column[col]->is_visible) col--;
                col=MAX(0, col);
            }
            gtk_sheet_click_cell(sheet, row, col, &veto);
            extend_selection = FALSE;
        ...
        case GDK_Tab:
            ...
            {
                col = col + scroll;
                while (col<sheet->maxcol && !sheet->column[col]->is_visible) col++;
        ...
        case GDK_Up:
            if (extend_selection)
            {
                ...
                if (sheet->selection_cell.row > 0)
                {
                    ...
                    while (row > 0 && !sheet->row[row].is_visible) row--;
                    row = MAX(0, row);
    ...
    gtk_sheet_recalc_text_column(GtkSheet *sheet, gint start_column)
{
    ...
    for (i=start_column+1; i<=sheet->maxcol; i++)  /* for the fresh columns */
    {
        ...
        if (i>0)  /* strange? */
        {
            sheet->column[i]->left_text_column = sheet->column[i-1]->left_text_column;
        }
#endif
    }
}
    ...
    gtk_sheet_size_allocate_entry(GtkSheet *sheet)
{
    ...
    gint row, col;
    gint size, max_size, text_size, column_width, row_height;
    const gchar *text;
    ...
    if (0 < sheet->active_cell.col && sheet->active_cell.col < sheet->maxcol)
    ...
    gtk_sheet_button_draw (GtkSheet *sheet, gint row, gint column)
{
    ...
    if (column >= 0)
    {
        ...
        if (!sheet->column_titles_visible) return;
        if (!sheet->column[column]->is_visible) return;
    ...
    if (button->label_visible)
    {
        ...
        gdk_gc_set_clip_rectangle(GTK_WIDGET(sheet)->style->fg_gc[button->state], &allocation);
...
*/
        ...
        gdk_gc_set_clip_rectangle(GTK_WIDGET(sheet)->style->fg_gc[button->state], NULL);
    ...
    AddColumns(GtkSheet *sheet, gint position, gint ncols)
{
    ...
    if (ncols > 0)
    {
        ...
        for (i=0;i<ncols;i++)
        {
            ...
            newobj = g_object_new(GTK_TYPE_SHEET_COLUMN, NULL);
